"""nuScenes 数据集 CenterPoint 单阶段 PointPillars 配置（Demo 演示版）。

与 10sweep 标准版结构一致，但 train/val 均指向 demo/nuScenes/demo_infos.pkl，用于离线演示/可视化推理。
0.2 米柱、点云范围 ±51.2 米、10 帧输入；热图损失权重 0.25。
"""
import itertools
import logging
...
# 推理后处理：过滤中心点合法范围，再做 NMS 去重并与置信度阈值比较。
test_cfg = dict(
