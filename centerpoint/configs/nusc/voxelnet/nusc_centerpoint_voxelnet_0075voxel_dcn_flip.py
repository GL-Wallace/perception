"""nuScenes 数据集 CenterPoint 单阶段 VoxelNet 配置（0.075 米体素、DCN 检测头 + 双翻转 TTA）。

CenterHead 使用可变形卷积（dcn_head=True），测试时开启双翻转 TTA（DOUBLE_FLIP=True）融合多方向推理结果。
...
DOUBLE_FLIP = True  # 测试时对点云做双重翻转 TTA，融合多方向推理结果 
...
model = dict(
    ...
    bbox_head=dict(
        ...
        dcn_head=True  # 检测头使用可变形卷积（DCN），增强几何形变建模能力
    ),
)
...
# 推理后处理：过滤中心点合法范围，再做旋转 NMS 去重并与置信度阈值比较（double_flip 控制融合方向数）。
test_cfg = dict(
