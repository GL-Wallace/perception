"""nuScenes 数据集 CenterPoint 单阶段 VoxelNet 配置（0.075 米体素、双翻转 TTA）。

使用 0.075 米体素、点云范围 ±54 米；测试时开启双翻转 TTA（DOUBLE_FLIP=True）融合多方向推理结果。
训练增强含全局旋转/缩放/平移噪声；训练 20 epoch，Adam + OneCycle（峰值 lr=0.001）。
"""
import itertools
import logging
