"""nuScenes 数据集 CenterPoint 单阶段 VoxelNet 配置（0.1 米体素）。

VoxelNet 主干 + CenterHead，按 nuScenes 官方 10 个任务的类别分组分别输出中心热图与回归；
体素 0.1x0.1x0.2 米，点云范围 ±51.2 米，叠加 10 帧点云（nsweeps=10、含速度特征）。
训练 20 epoch，Adam + OneCycle（峰值 lr=0.001）；热图损失权重 0.25。
"""
import itertools
import logging
