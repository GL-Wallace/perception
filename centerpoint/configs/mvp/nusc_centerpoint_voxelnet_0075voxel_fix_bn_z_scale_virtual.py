"""nuScenes 数据集 CenterPoint 单阶段 VoxelNet 配置（MVP 变体、虚拟点、z 轴修正）。

reader 使用 DynamicVoxelEncoder 并启用虚拟点（virtual=True），backbone 输入 21 维特征（含虚拟点标签）；
...
# 模型配置：reader 动态体素化输入点云（含虚拟点），backbone 稀疏卷积下采样，neck 融合多尺度，bbox_head 输出中心热图与回归。
model = dict(
    type="VoxelNet",
    pretrained=None,
    reader=dict(
...
# 数据加载器配置：每卡 batch 大小与数据加载线程数，以及 train/val/test 三个数据集实例（virtual=True 加载虚拟点）。
data = dict(
    samples_per_gpu=4,
    workers_per_gpu=4,
    train=dict(
