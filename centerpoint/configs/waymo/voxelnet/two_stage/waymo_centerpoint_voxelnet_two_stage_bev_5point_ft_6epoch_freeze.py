"""Waymo 数据集 CenterPoint 两阶段 VoxelNet 配置（单帧、无速度、二阶段微调）。

第一阶段冻结 waymo_centerpoint_voxelnet_3x 权重（freeze=True、code_size=7）；
第二阶段 BEVFeatureExtractor 采样 5 个面中心特征，RoIHead 预测 IoU 置信度并精炼框（论文 Sec. 3.4）。
单帧输入（nsweeps=1）、体素 0.1 米；微调 6 epoch，Adam + OneCycle（峰值 lr=0.003）。
"""
import itertools
import logging
