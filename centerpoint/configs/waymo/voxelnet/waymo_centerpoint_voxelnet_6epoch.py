"""Waymo 数据集 CenterPoint 单阶段 VoxelNet 配置（6 epoch 训练）。

VoxelNet 主干（VoxelFeatureExtractorV3 + SpMiddleResNetFHD + RPN）+ CenterHead；
检测 VEHICLE/PEDESTRIAN/CYCLIST 三类，体素 0.1x0.1x0.15 米，点云范围 ±75.2 米，单帧输入（nsweeps=1）。
训练 6 epoch，Adam + OneCycle（峰值 lr=0.003）。
"""
import itertools
import logging
